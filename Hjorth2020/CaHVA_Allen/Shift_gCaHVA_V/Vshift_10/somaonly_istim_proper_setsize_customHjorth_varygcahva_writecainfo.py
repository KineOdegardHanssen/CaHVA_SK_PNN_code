...
if __name__ == '__main__':
    ...
    gnaf    = 1.0
    gkaf    = 1.0
    gcahvas = [0.2,0.06,0.08,0.12,0.14,0.16,0.18,0.25]
    gpas    = 1.0
    Ng = len(gcahvas)
    ...
    for i in range(Ni):
        ...
        for j in range(Ng):
            ...
            t, v, eca, cai, cao, I_Ca_HVA, g_Ca_HVA = return_allen_cell_model(model_folder,cm_factor,somasize,idur,iamp,idelay,tstop,dtexp,gnaf,gkaf,gcahva,gpas,vary_naf,vary_kaf,vary_Ca_HVA,vary_gpas,varyE,varymech,v_init,t_before_rec)
            
            # Save results:
            filename = folder+namestring+'somaonly_cm'+str(cm_factor)+'_idur%i_iamp'%idur+str(iamp)+'_dtexp%i_vinit' % dtexp+str(v_init)+'_trec'+str(t_before_rec)+'_V_cainfo.txt'
            
            Nt = len(t)
            file = open(filename,'w')
            for k in range(Nt):
                file.write('%.16e %.16e %.16e %.16e %.16e %.16e %.16e\n' % (t[k],v[k],eca[k],cai[k],cao[k],I_Ca_HVA[k],g_Ca_HVA[k]))
            file.close()   
